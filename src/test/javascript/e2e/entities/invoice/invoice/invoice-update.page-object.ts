...
export default class InvoiceUpdatePage {
  ...
  productOrderIdInput: ElementFinder = element(by.css('input#invoice-productOrderId'));
  ...
  async setProductOrderIdInput(productOrderId) {
    await this.productOrderIdInput.sendKeys(productOrderId);
  }

  async getProductOrderIdInput() {
    return this.productOrderIdInput.getAttribute('value');
  }
  ...
  async enterData() {
    ...
    await waitUntilDisplayed(this.saveButton);
    await this.setProductOrderIdInput('5');
    expect(await this.getProductOrderIdInput()).to.eq('5');
    await this.save();
